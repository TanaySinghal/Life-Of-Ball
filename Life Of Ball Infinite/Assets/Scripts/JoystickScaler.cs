...
public class JoystickScaler : MonoBehaviour {
	...
	void Awake () {
		float multiplier = 1f;
		float movementRangeMultiplier = 1f;
		if(Screen.height > 600) {
			multiplier = 1.5f;
			movementRangeMultiplier = 1.5f;
		}
		if(Screen.height > 1000) {
			multiplier = 2.5f;
			movementRangeMultiplier = 2f;
		}
		if(Screen.height > 1400) {
			multiplier = 3.5f;
			movementRangeMultiplier = 3f;
		}
		//Menu.multiplier
		...
		if(joystick != null) {
			joystick.MovementRange = (int)(joystick.MovementRange*movementRangeMultiplier);
		}
