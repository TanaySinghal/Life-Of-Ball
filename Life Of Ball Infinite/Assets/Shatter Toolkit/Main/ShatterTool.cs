...
public class ShatterTool : MonoBehaviour
{
	...
	private Transform shatteredObjectParent;
	...
	public void Awake()
	{
		...
		shatteredObjectParent = GameObject.Find("ShatteredPlatforms").transform;
	}
	...
	private void CreateNewGameObjects(IList<Hull> newHulls, out GameObject[] newGameObjects)
	{
		...
		for (int i = 0; i < newHulls.Count; i++)
		{
			...
			//Change tags
			newGameObject.tag = "ShatteredPlatform";
			...
			//Add drown platform script
			newGameObject.AddComponent<DrownPlatform>();

			//Change parent
			newGameObject.name = "Shattered Pieces";
			newGameObject.transform.parent = shatteredObjectParent;
			newGameObjects[i] = newGameObject;
		}

		Debug.Log ("Played sound");
		//Only add explosion force once
	}
}